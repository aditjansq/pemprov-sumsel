...
// 1. Import Data Seed
import { seedPengepul } from "../data/seedPengepul";

import { 
  Plus, Search, Pencil, Trash2, Eye,
  ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight,
  ArrowLeft, Save, Upload, MapPin, Phone, User, X, Box, ImageIcon
} from 'lucide-react';

export default function Pengepul() {
  // --- STATE GLOBAL ---
  const [view, setView] = useState('list'); // 'list' | 'create' | 'detail'
  const [selectedItem, setSelectedItem] = useState(null);

  // --- DATA DUMMY (Dari File Seed) ---
  const [dataPengepul] = useState(seedPengepul);
  ...
  // --- LOGIC FILTERING ---
  const filteredData = dataPengepul.filter((item) =>
  ...
  const renderListView = () => (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-gray-800">Data Pengepul</h2>
          <p className="text-sm text-gray-500">Kelola data pengepul komoditas di sini.</p>
        </div>
        ...
        <table className="w-full text-sm text-left text-gray-500">
          ...
          <tbody>
            {currentItems.length > 0 ? (
              ...
              <tr><td colSpan="6" className="px-6 py-8 text-center text-gray-500">Data pengepul tidak ditemukan.</td></tr>
            )}
          </tbody>
        </table>
      </div>
  ...
  const renderCreateView = () => (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-5 border-b border-gray-100 flex items-center gap-3 bg-gray-50">
        ...
        <h2 className="text-lg font-bold text-gray-800">Tambah Data Pengepul</h2>
      </div>
      ...
      <div className="p-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input type="text" className="w-full pl-10 border border-gray-300 rounded-lg p-2.5 focus:border-green-500 focus:ring-green-500 outline-none transition bg-white" placeholder="Masukkan Nama Pengepul"/>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <div className="absolute top-3 left-3 flex items-center pointer-events-none text-gray-400"><MapPin size={18} /></div>
                <textarea rows="4" className="w-full pl-10 border border-gray-300 rounded-lg p-2.5 focus:border-green-500 focus:ring-green-500 outline-none transition bg-white resize-none" placeholder="Masukkan Alamat Lengkap"></textarea>
              </div>
            </div>

            <div className="space-y-2">
              <label className="block text-sm font-bold text-gray-700">Telepon / WA</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-gray-400"><Phone size={18} /></div>
                <input type="tel" className="w-full pl-10 border border-gray-300 rounded-lg p-2.5 focus:border-green-500 focus:ring-green-500 outline-none transition bg-white" placeholder="08xxxxxxxx"/>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="block text-sm font-bold text-gray-700">Komoditas</label>
              <div className="relative">
                  <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-gray-400"><Box size={18} /></div>
                  <select className="w-full pl-10 border border-gray-300 rounded-lg p-2.5 focus:border-green-500 focus:ring-green-500 outline-none transition bg-white text-gray-700 appearance-none">
            ...
            {/* Unggah Foto */}
            <div className="space-y-2">
              <label className="block text-sm font-bold text-gray-700">Unggah Foto Pengepul</label>
  ...
  const renderDetailView = () => (
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
      ...
      <div className="px-8 py-6 border-b border-gray-100 bg-white flex items-center gap-4 sticky top-0 z-10">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900 tracking-tight">Detail Pengepul</h2>
          <p className="text-sm text-gray-500">Informasi lengkap profil dan komoditas pengepul.</p>
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="lg:col-span-1">
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col items-center text-center h-full">
                 ...
                 <p className="text-sm text-gray-500 font-medium mb-6">Pengepul Terdaftar</p>
           ...
           <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                 <h4 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-5 border-b border-gray-100 pb-3">Informasi Bisnis</h4>
                 <div className="flex items-start gap-4">
  ...
  return (
    <MainLayout title={view === 'list' ? "Data Pengepul" : view === 'create' ? "Tambah Pengepul" : "Detil Pengepul"}>
