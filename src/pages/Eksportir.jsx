...
// 1. Import Data Seed
import { seedEksportir } from "../data/seedEksportir";

import { 
  Plus, Search, Pencil, Trash2, Eye,
  ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight,
  ArrowLeft, Upload, Download, Save, X,
  Building2, MapPin, Phone, User, FileText, CloudUpload, ImageIcon
} from 'lucide-react';

export default function Eksportir() {
  // --- STATE GLOBAL ---
  const [view, setView] = useState('list'); 
  const [selectedItem, setSelectedItem] = useState(null);

  // --- DATA DUMMY (Diambil dari file eksternal) ---
  const [dataEksportir] = useState(seedEksportir);
  ...
  // --- LOGIC FILTERING ---
  const filteredData = dataEksportir.filter((item) =>
  ...
  const renderListView = () => (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-gray-800">Data Eksportir</h2>
          <p className="text-sm text-gray-500">Kelola data eksportir komoditas unggulan.</p>
        </div>
      ...
      <div className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gray-50/50">
        ...
        <div className="relative">
          ...
          <input type="text" value={searchTerm} onChange={handleSearch} className="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full sm:w-64 pl-10 p-2" placeholder="Cari Nama atau Perusahaan..." />
        </div>
      </div>
        ...
        <table className="w-full text-sm text-left text-gray-500">
          <thead className="text-xs text-gray-700 uppercase bg-gray-100 border-b border-gray-200">
            <tr>
              ...
              <th className="px-6 py-4 font-semibold border-r border-gray-200">Nama Perusahaan</th>
            ...
            {currentItems.length > 0 ? (
              currentItems.map((item, index) => (
                <tr key={item.id} className="bg-white border-b border-gray-100 hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4 text-green-700 font-medium border-r border-gray-200">{item.perusahaan}</td>
              ...
              <tr><td colSpan="6" className="px-6 py-8 text-center text-gray-500">Data eksportir tidak ditemukan.</td></tr>
            )}
          </tbody>
        </table>
      </div>
  ...
  const renderCreateView = () => (
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
      ...
      <div className="px-8 py-6 border-b border-gray-100 bg-white flex items-center gap-4 sticky top-0 z-10">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900 tracking-tight">Tambah Data Eksportir</h2>
          <p className="text-sm text-gray-500">Lengkapi form di bawah untuk mendaftarkan eksportir baru.</p>
        </div>
      </div>
      ...
      <div className="p-8 bg-gray-50/30">
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* KOLOM KIRI (Data Tekstual) */}
          <div className="lg:col-span-2 space-y-6">
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">Nama Eksportir <span className="text-red-500">*</span></label>
              <div className="relative">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">Nama Perusahaan <span className="text-red-500">*</span></label>
              <div className="relative">
                 <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-gray-400"><Building2 size={18} /></div>
                 <input type="text" className="w-full pl-11 px-4 py-3 rounded-xl border border-gray-300 bg-white text-gray-900 placeholder-gray-400 focus:border-green-500 focus:ring-4 focus:ring-green-500/10 outline-none transition-all duration-200 font-medium" placeholder="Contoh: PT. Sriwijaya Makmur"/>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                 ...
                 <textarea rows="3" className="w-full pl-11 px-4 py-3 rounded-xl border border-gray-300 bg-white text-gray-900 placeholder-gray-400 focus:border-green-500 focus:ring-4 focus:ring-green-500/10 outline-none transition-all duration-200 font-medium resize-none" placeholder="Masukkan Alamat Perusahaan"></textarea>
              </div>
            </div>

            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">Telepon / WhatsApp <span className="text-red-500">*</span></label>
              <div className="relative">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
              <label className="block text-sm font-semibold text-gray-700 mb-4">Foto Profil / Logo</label>
            ...
            <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
              <label className="block text-sm font-semibold text-gray-700 mb-4">Dokumen Legal (PDF)</label>
              <input type="file" ref={docInputRef} onChange={handleDocChange} accept=".pdf" className="hidden" />
              <div onClick={() => docInputRef.current.click()} className={`group border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer transition-all duration-300 ${docFile ? 'border-green-500 bg-green-50/20' : 'border-gray-300 hover:border-green-500 hover:bg-green-50/30'}`}>
                <div className={`p-3 rounded-full mb-2 transition-all duration-300 ${docFile ? 'bg-green-100' : 'bg-gray-50 group-hover:scale-110 group-hover:bg-white group-hover:shadow-md'}`}>
                  {docFile ? <FileText size={24} className="text-green-600" /> : <CloudUpload size={24} className="text-gray-400 group-hover:text-green-600" />}
                </div>
                <p className={`text-xs font-semibold text-center ${docFile ? 'text-green-700' : 'text-gray-500 group-hover:text-green-700'}`}>{docFile ? docFile.name : "Upload PDF Legalitas"}</p>
              </div>
            </div>
  ...
  const renderDetailView = () => (
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
      ...
      <div className="px-8 py-6 border-b border-gray-100 bg-white flex items-center gap-4 sticky top-0 z-10">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900 tracking-tight">Detail Eksportir</h2>
          <p className="text-sm text-gray-500">Informasi lengkap profil dan legalitas eksportir.</p>
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="lg:col-span-1">
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col items-center text-center h-full">
                 <div className="w-40 h-40 mb-6 relative">
                    ...
                    <div className="absolute bottom-2 right-2 bg-green-500 text-white p-1.5 rounded-full border-2 border-white shadow-sm" title="Verified">
                       <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>
                    </div>
                 </div>
                 <h3 className="text-xl font-bold text-gray-900 mb-1">{selectedItem?.nama || "-"}</h3>
                 <p className="text-sm text-gray-500 font-medium mb-6">Eksportir Terdaftar</p>
           ...
           <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                 <h4 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-5 border-b border-gray-100 pb-3">Informasi Perusahaan</h4>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1"><label className="text-xs font-medium text-gray-500 flex items-center gap-1"><Building2 size={14} /> Nama Perusahaan</label><p className="text-base font-semibold text-gray-900">{selectedItem?.perusahaan || "-"}</p></div>
                    <div className="space-y-1"><label className="text-xs font-medium text-gray-500 flex items-center gap-1"><Phone size={14} /> Kontak / WhatsApp</label><p className="text-base font-semibold text-gray-900 font-mono">{selectedItem?.telepon || "-"}</p></div>
                    <div className="space-y-1 md:col-span-2"><label className="text-xs font-medium text-gray-500 flex items-center gap-1"><MapPin size={14} /> Alamat Lengkap</label><p className="text-base font-semibold text-gray-900 leading-relaxed">{selectedItem?.alamat || "-"}</p></div>
                 </div>
              </div>

              {/* Card: Dokumen Legalitas */}
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                 <h4 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-5 border-b border-gray-100 pb-3">Berkas Legalitas</h4>
                 <div className="flex items-center justify-between p-4 rounded-xl border border-gray-200 hover:border-green-500 hover:shadow-md hover:bg-green-50/30 transition-all duration-300 group">
                    <div className="flex items-center gap-4">
                       <div className="p-3 bg-red-50 text-red-600 rounded-lg group-hover:scale-110 transition-transform"><FileText size={24} /></div>
                       <div><p className="text-sm font-bold text-gray-900 mb-0.5">Dokumen Legal Perusahaan</p><p className="text-xs text-gray-500 group-hover:text-green-700 transition-colors">PDF • 2.4 MB • Diunggah 12 Jan 2025</p></div>
                    </div>
  ...
  return (
    <MainLayout title={view === 'list' ? "Data Eksportir" : view === 'create' ? "Tambah Eksportir" : "Detil Eksportir"}>
