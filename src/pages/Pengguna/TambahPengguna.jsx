...
// --- KOMPONEN BANTUAN UNTUK MERINGKAS INPUT FIELDS ---

/**
 * Komponen Input/Textarea generik
 * Mengelola ikon, state error, dan toggle password
 */
const InputField = ({
  label,
  name,
  ...
  const IconComponent = Icon ? <Icon size={18} /> : null;
...
/**
 * Komponen Select Field generik (multi-select)
 */
const SelectField = ({
  label,
  name,
...
export default function TambahPengguna() {
  ...
  const validate = () => {
    ...
    if (!formData.kata_sandi || formData.kata_sandi.length < 6) {
    ...
    if (!formData.konfirmasi_kata_sandi) {
      ...
      newErrors.konfirmasi_kata_sandi = "Kata sandi dan konfirmasi tidak cocok.";
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Navigasi setelah notifikasi selesai ditampilkan
      setTimeout(() => {
        navigate("/pengguna");
      }, 3000); 
  ...
  return (
    <MainLayout title="Tambah Pengguna">
      <ToastContainer /> {/* Komponen Toastify ditempatkan di sini */}
        ...
        <div className="p-8 bg-gray-50/30">
          <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            ...
            <div className="space-y-6">
              <InputField
                label="Nama Lengkap" name="nama_lengkap" onChange={handleChange}
                error={errors.nama_lengkap} icon={User} placeholder="Masukkan Nama Lengkap"
              />
              <InputField
                label="Email (Wajib)" name="email" type="email" onChange={handleChange}
                error={errors.email} icon={User} placeholder="contoh@email.com"
              />
              <InputField
                label="Alamat" name="alamat" onChange={handleChange}
                error={errors.alamat} icon={MapPin} placeholder="Masukkan Alamat Lengkap"
                isTextArea rows={3}
              />
              <div className="pt-4">
                <InputField
                  label="Telepon / WA" name="telepon_wa" type="tel" onChange={handleChange}
                  error={errors.telepon_wa} icon={Phone} placeholder="08xxxxxxxx"
                />
              </div>
            </div>
            ...
            <div className="space-y-6 flex flex-col">
              <InputField
                label="Nama Pengguna" name="nama_pengguna" onChange={handleChange}
                error={errors.nama_pengguna} icon={User} placeholder="Masukkan Username"
              />
              <InputField
                label="Kata Sandi" name="kata_sandi" type="password" onChange={handleChange}
                error={errors.kata_sandi} icon={Lock} placeholder="Masukkan Kata Sandi"
                onTogglePassword={() => setShowPassword((prev) => !prev)} showPassword={showPassword}
              />
              <InputField
                label="Konfirmasi Kata Sandi" name="konfirmasi_kata_sandi" type="password" onChange={handleChange}
                error={errors.konfirmasi_kata_sandi} icon={Lock} placeholder="Ulangi Kata Sandi"
                onTogglePassword={() => setShowPasswordConfirm((prev) => !prev)} showPassword={showPasswordConfirm}
              />
              <SelectField
                label="Hak Akses" name="hak_akses" options={optionsHakAkses}
                onChange={handleSelectChange} error={errors.hak_akses} icon={Shield}
                customStyles={customStyles} placeholder="Pilih Hak Akses (Bisa Banyak)"
                hint="Anda dapat memilih lebih dari satu hak akses."
              />
              <InputField
                label="Instansi" name="instansi" onChange={handleChange}
                icon={Building2} placeholder="Nama Instansi"
              />

              {/* Tombol Aksi */}
              <div className="flex justify-end pt-4 mt-auto gap-3">
                <button
                  onClick={() => navigate("/pengguna")} disabled={isLoading}
