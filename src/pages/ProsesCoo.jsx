...
export default function SertifikatCoo() {
  ...
  // --- LOGIC FILTERING ---
  const filteredData = dataCOO.filter((item) =>
  ...
  const handleGoToUpload = (item) => {
    ...
    setCooFile(null); // Reset file saat masuk halaman upload
    setView('upload');
  };
  ...
  const handleToggleStatus = (id, field) => {
      const updatedData = dataCOO.map(item => {
          if(item.id === id) {
              return { ...item, [field]: !item[field] };
          }
          return item;
      });
      setDataCOO(updatedData);
  };
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="bg-orange-50 border-l-4 border-orange-500 p-4 m-6 mb-2 flex items-center gap-3 rounded-r-lg">
         ...
         <p className="text-sm font-bold text-gray-800">
           Terdapat {dataCOO.filter(item => !item.cooCompleted).length} Proses Ekspor yang Belum Mendapatkan Certificate of Origin (COO)
         </p>
      </div>
              ...
              currentItems.map((item, index) => (
                <tr key={item.id} className="bg-white border-b border-gray-100 hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4 border-r border-gray-200 text-center">
                    {item.cooCompleted ? (
                      <button onClick={() => handleToggleStatus(item.id, 'cooCompleted')} title="Sudah Selesai">
                  ...
                  <td className="px-6 py-4 border-r border-gray-200 text-center">
                     <button onClick={() => handleToggleStatus(item.id, 'healthCertCompleted')} title={item.healthCertCompleted ? "Sudah Selesai" : "Belum Selesai"}>
  ...
  // =========================================
  // 2. TAMPILAN UPLOAD (TAMBAH SERTIFIKAT COO - MODERN UI)
  // =========================================
  const renderUploadView = () => (
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
      ...
      <div className="px-8 py-6 border-b border-gray-100 bg-white flex items-center gap-4 sticky top-0 z-10">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900 tracking-tight">Tambah Sertifikat COO</h2>
          <p className="text-sm text-gray-500">Lengkapi dokumen Certificate of Origin untuk proses ekspor ini.</p>
        </div>
      </div>
               ...
               <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                  ...
                  <div className="space-y-6">
                     <div className="space-y-2">
                        <label className="block text-sm font-semibold text-gray-700">Certificate of Origin (COO)</label>
                        <input type="file" ref={cooInputRef} onChange={handleFileChange} accept=".pdf" className="hidden" />
                        
                        <div 
                           onClick={() => cooInputRef.current.click()} 
                           className={`group border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer transition-all duration-300 ${cooFile ? 'border-green-500 bg-green-50/20' : 'border-gray-300 hover:border-green-500 hover:bg-green-50/30'}`}
                        >
                           <div className={`p-3 rounded-full mb-2 transition-all duration-300 ${cooFile ? 'bg-green-100' : 'bg-gray-50 group-hover:scale-110 group-hover:bg-white group-hover:shadow-md'}`}>
                              {cooFile ? <FileText size={24} className="text-green-600" /> : <CloudUpload size={24} className="text-gray-400 group-hover:text-green-600" />}
                           </div>
                           <p className={`text-xs font-semibold text-center ${cooFile ? 'text-green-700' : 'text-gray-500 group-hover:text-green-700'}`}>
                              {cooFile ? cooFile.name : "Klik untuk Unggah Sertifikat (.pdf)"}
                           </p>
                        </div>
                     </div>
                     ...
                     <div className="flex justify-end gap-3 pt-2">
                        ...
                        <button 
                           onClick={() => {
                               ...
                               handleToggleStatus(selectedItem.id, 'cooCompleted');
                               handleBack();
                           }} 
  ...
  return (
    <MainLayout title={view === 'list' ? "Sertifikat COO" : "Tambah Sertifikat COO"}>
